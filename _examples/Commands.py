...
Força!! :muscle::wink: ''')
    ...
    async def matan_command(self, ctx):
        await ctx.send("Ask me what you want, I'll give you the answer")

    @commands.command(name="arthur")
    async def arthur_command(self, ctx):
        await ctx.send("I'll eat glass and cry in the shower after this....:smiling_face_with_tear:")

    @commands.command(name="ander")
    async def ander_command(self, ctx):
        await ctx.send("Good cxr, you deserve it")

    @commands.command(name="kyle")
    async def kyle_command(self, ctx):
        await ctx.send("And let's go to the off topic, this chat was totally distorted")

    @commands.command(name="water")
    async def agua_command(self, ctx):
        await ctx.send("--> Drink water <--")
    ...
    async def on_command_error(self, ctx, error):
        if isinstance(error, commands.error.CommandNotFound):
            await ctx.send("This command does not exist! :alien:")
        else:
            raise error
